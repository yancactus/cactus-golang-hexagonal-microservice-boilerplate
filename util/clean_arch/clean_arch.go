...
func (v *Validator) Validate(root string, ignoreTests bool, ignoredPackages []string) (count int, pass bool, validationErrors []ValidationError, err error) {
	...
	ImportsLoop:
		for _, imp := range f.Imports {
			...
			importErrors := v.validateImport(imp, importerMeta, path)
	...
	return count, len(validationErrors) == 0, validationErrors, nil
}
