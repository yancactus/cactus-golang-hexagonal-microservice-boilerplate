...
func TestConfigEnvOverrides(t *testing.T) {
	...
	_ = os.Setenv("APP_POSTGRES_HOST", "test-postgres-host")
	_ = os.Setenv("APP_POSTGRES_PORT", "5433")
	...
	defer func() {
		...
		_ = os.Unsetenv("APP_POSTGRES_HOST")
		_ = os.Unsetenv("APP_POSTGRES_PORT")
	...
	assert.Equal(t, "test-postgres-host", conf.Postgre.Host)
	assert.Equal(t, 5433, conf.Postgre.Port)
...
`
	...
	err = os.WriteFile(configFile, []byte(updatedConfigContent), 0o644)
	require.NoError(t, err)
	...
	assert.Equal(t, "updated-host", updatedConfig.Postgre.Host)
	assert.Equal(t, 5433, updatedConfig.Postgre.Port)
